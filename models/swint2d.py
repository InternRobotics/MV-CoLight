...
@MODELS.register_module("SwinT2D")
class SwinT2D(nn.Module):
    def __init__(
        self, 
        num_in_ch=7,
        num_out_ch=3,
        ...
        **kwargs):
        super(SwinT2D, self).__init__()
        ################################### 1. Feature Extraction Network ###################################
        # coarse feature
        self.conv_first = nn.Sequential(  
            nn.Conv2d(num_in_ch, embed_dim, kernel_size=3, bias=True, stride=1, padding=1, padding_mode='reflect'),
        ...
        self.conv_end = nn.Conv2d(embed_dim, num_out_ch, 3, 1, 1)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.conv_end(res + self.conv_skip(x))
        x = x.float()
        x = torch.sigmoid(x)
        x = x.reshape(B, N, -1, H, W)
        return x
