...
class DTCMultiLightDataset(Dataset):
    ...
    def load_jsons(self, data_id, scene_path):
        ...
        if os.path.exists(os.path.join(scene_path, "transforms.json")):
            frame, num_cd, flag = self.load_test_json(scene_path)
        else:
            frame, num_cd, flag = self.load_train_json(scene_path)
        return frame, scene_id, num_cd, flag

    def load_test_json(self, scene_path):
        ...
        composite_paths, depth_paths, gt_paths, bg_paths, mask_paths = [], [], [], [], []
        for i, frame in enumerate(data["frames"]):
            composite_paths.append(os.path.join(data_root, frame["composite_path"]))
            bg_paths.append(os.path.join(data_root, frame["bg_path"]))
            depth_paths.append(os.path.join(data_root, frame["depth_path"]))
            gt_paths.append(os.path.join(data_root, frame["gt_path"]))
    ...
    def load_train_json(self, scene_path):
        scene_frames = []
        data_root = os.path.dirname(scene_path)
        for light_cd in sorted(os.listdir(scene_path)):
            ...
            image_paths, depth_paths, object_mask_paths, object_bg_paths = [], [], [], []
            for i, frame in enumerate(data["frames"]):
    ...
    def augment(self, context_images, context_bg_images, context_depths, target_images):
        
        if np.random.rand() < 0.5:
            context_images = torch.flip(context_images, [-1])
            context_bg_images = torch.flip(context_bg_images, [-1])
            context_depths = torch.flip(context_depths, [-1])
            target_images = torch.flip(target_images, [-1])

        return context_images, context_bg_images, context_depths, target_images
    ...
    def getitem(self, index):
        ...
        if flag:
            ...
            random_indices = torch.randperm(len(base_example['composite_path']))
        ...
        if self.stage == 'train':
            context_images, context_bg_images, context_depths, target_images = self.augment(
                context_images, context_bg_images, context_depths, target_images)
